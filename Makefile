...
MAN_PAGES = $(MANS:.md=.1)
LIB = "helper/env-reset" "helper/gw-utils"

COMMANDS_USED_WITHOUT_GIT_REPO = git-work
...
install:
	...
	$(eval TEMPFILE := $(shell mktemp -q ${TMPDIR:-/tmp}/git-work.XXXXXX 2>/dev/null || mktemp -q))
	@chmod +x $(TEMPFILE)
	...
	@if [ -z "$(wildcard man/git-*.1)" ]; then \
		...
		echo "... adding git-*.1 to man pages"; \
	fi
	@mkdir -p $(DESTDIR)/etc/bash_completion.d
	cp -f etc/bash_completion.sh $(DESTDIR)/etc/bash_completion.d/git-work
...
uninstall:
	...
	rm -f $(DESTDIR)/etc/bash_completion.d/git-work
